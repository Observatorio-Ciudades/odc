...
class PCRasterData():
    ...
    projection_crs:str="EPSG:6372"
    compute_unavailable_dates:bool=True
    catalog:str="https://planetarycomputer.microsoft.com/api/stac/v1"
    missing_months_pct_limit:int=50
    continuous_missing_months_limit:int=6
    buffer_radio:int=500
    download_time_limit:int=1500


    def __init__(self, gdf, index_analysis, area_of_analysis_name, freq, start_date, end_date,
        tmp_dir, band_name_dict, index_equation, query, satellite, projection_crs,
        compute_unavailable_dates, catalog, missing_months_pct_limit,
        buffer_radio, download_time_limit):
        self.gdf = gdf
        ...
        self.band_name_dict = band_name_dict
        self.index_equation = index_equation
        self.query = query
        self.satellite = satellite
        self.projection_crs = projection_crs
        self.compute_unavailable_dates = compute_unavailable_dates
        self.catalog = catalog
        self.missing_months_pct_limit = missing_months_pct_limit
        self.buffer_radio = buffer_radio
        self.download_time_limit = download_time_limit

        return None

    def download_raster_from_pc(self):
        """
        ...
        # area_of_interest = create_area_of_interest()
        ...
        # time_of_interest = create_time_of_interest(self.start_date, self.end_date, freq=self.freq)
        ...
        # items = gather_items(time_of_interest, self.area_of_interest, query=self.query, satellite=self.satellite)
        self.gather_items()
        ...
        # date_list = available_datasets(self.items, self.satellite, self.query)
        ...
        df_len = self.create_raster_by_month(df_len, gdf_raster_test, gdf_bb)
    ...
    def check_available_data(self, df_len):
        pct_missing = round(self.missing_months/len(df_len),2)*100
        log(f'Created DataFrame with {self.missing_months} ({pct_missing}%) missing months')
        if pct_missing >= self.pct_limit:
            raise AvailableData('Missing more than 50 percent of data points')
        df_rol = df_len.rolling(self.window_limit).sum()
        if len(df_rol.loc[df_rol.data_id==0])>0:
            raise AvailableData('Multiple missing months together')
        del df_rol

    def create_area_of_interest(self):

        # Create buffer around hexagons
        poly = self.gdf.to_crs(self.projection_crs).buffer(self.buffer_radio)
        ...
        #
    def create_time_of_interest(self):
        """
    ...
    def create_raster_by_month(self, df_len, gdf_raster_test, gdf_bb):
        """
        ...
        df_file_dir = self.define_processing_directory(df_len)
        ...
        for i in tqdm(range(len(df_len)), position=0, leave=True):
            ...
            if self.check_preexisting_files(i, df_file_dir, df_raster):
                continue
            ...
            self.download_and_process_raster_by_date(gdf_bb, gdf_raster_test)
    ...
    def define_processing_directory(self, df_len):
        # check if file exists, for example in case of code crash
        df_file_dir = self.tmp_dir + self.index_analysis + f'_{self.area_of_analysis_name}_dataframe.csv'
        if os.path.exists(df_file_dir) == False: # Or folder, will return true or false
            df_len.to_csv(df_file_dir, index=False)
        # create folder to store output rasters during execution
        ...
        # create folder to store temporary processing rasters
        if os.path.exists(self.processing_raster_dir) == False: # Or folder, will return true or false
            os.mkdir(self.processing_raster_dir)

        return df_file_dir


    def check_preexisting_files(self, i, df_file_dir, df_raster):
        ...
        if f'{self.area_of_analysis_name}_{self.index_analysis}_{self.month_}_{self.year_}.tif' in os.listdir(self.tmp_dir_name):
    ...
    def raster_download_date_preprocessing(self):
    ...
    def mosaic_process(self, raster_bands, gdf_bb):
        ...
        for b in band_names_list:
            ...
            raster_array[b]= [mosaic_raster(raster_bands[b], self.processing_raster_dir,
                                           upscale=self.band_name_dict[b][0])]
            ...
            with rasterio.open(f"{self.processing_raster_dir}{b}.tif", "w", **raster_array[b][2]) as dest:
            ...
            with rasterio.open(f"{self.processing_raster_dir}{b}.tif") as src:
    ...
    def calculate_raster_index(self, raster_arrays):
        """
    ...
    def raster_nan_test(self, gdf, raster_file):
        """
    ...
    def clean_mask(self, geom, dataset='', **mask_kw):
        """
    ...
    def download_and_process_raster_by_date(self, gdf_bb, gdf_raster_test):
        ...
        while iter_count <= 5:
            ...
            for data_link in range(len(self.dates_ordered)):
                ...
                try:
                    ...
                    with rasterio.open(f"{self.processing_raster_dir}{self.index_analysis}.tif",'w', **out_meta) as dest:
                    ...
                    try:
                        ...
                        self.raster_nan_test(gdf_raster_test, raster_file)

                        log('Passed null test')

                        # save raster to processing database
                        index_raster_dir = f"{self.processing_raster_dir}{self.area_of_analysis_name}_{self.index_analysis}_{self.month_}_{self.year_}.tif"
                        with rasterio.open(index_raster_dir,'w', **out_meta) as dest:
                            dest.write(raster_fill)
                        ...
                        checker = 1
                        iter_count = 6
                        delete_files_from_folder(self.processing_raster_dir)
                        break
                    except:
