...
def plot_ndvi(data_gdf: gpd.GeoDataFrame,
    ...
    # (e.g. columns 'max_preescolar' or 'median_time', which indicates average time (minutes) data and is categorized in time bins)
        ...
        data_gdf.loc[data_gdf[column]>=0.6 , f'{column}_cat'] = 'High vegetation density'
        data_gdf.loc[(data_gdf[column]>=0.4 )&
                     (data_gdf[column]<0.6), f'{column}_cat'] = 'Moderate vegetation density'
        data_gdf.loc[(data_gdf[column]>=0.2)&
                     (data_gdf[column]<0.4), f'{column}_cat'] = 'Low vegetation density'
        data_gdf.loc[(data_gdf[column]>=0.1)&
                     (data_gdf[column]<0.2), f'{column}_cat'] = 'Bare soil'
        data_gdf.loc[(data_gdf[column]<0.1), f'{column}_cat'] = 'Artificial surface/Water/Rock'
        # Order data
        categories = ['Artificial surface/Water/Rock', 'Bare soil', 'Low vegetation density', 'Moderate vegetation density', 'High vegetation density']
        data_gdf[f'{column}_cat'] = pd.Categorical(data_gdf[f'{column}_cat'], categories=categories, ordered=True)
        # --- PLOT
        # Plot proximity data using the viridis color palette reversed
