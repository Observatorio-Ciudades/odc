...
def calculate_distance_nearest_poi(
    ...
    g, weights, node_mapping = to_igraph(nodes,edges,weight=weight) #convert to igraph to run the calculations
...
def proximity_isochrone(
    ...
    nodes_distance_prep = calculate_distance_nearest_poi(nearest, nodes_analysis, edges,
        poi_name,'osmid', weight='time_min',count_pois=count_pois)
...
def proximity_isochrone_from_osmid(
    ...
    hull_geometry = proximity_isochrone(G,
        nodes,
        edges,
        point_of_interest,
        trip_time,
...
def pois_time(
    ...
    """
    # Validate inputs
    _validate_pois_time_inputs(G, nodes, edges, pois, poi_name, prox_measure, walking_speed)
...
def id_pois_time(
    ...
    """
    # Validate inputs
    _validate_id_pois_time_inputs(G, nodes, edges, pois, poi_name, prox_measure,
                                  walking_speed, goi_id)
...
def _validate_pois_time_inputs(G, nodes, edges, pois, poi_name, prox_measure, walking_speed):
...
def _validate_id_pois_time_inputs(G, nodes, edges, pois, poi_name, prox_measure,
                                  walking_speed, goi_id):
...
def _handle_empty_pois(nodes: GeoDataFrame, poi_name: str, count_pois: Tuple[bool, int],
                       use_nan: bool = False) -> GeoDataFrame:
    ...
    if count_pois[0]:
        ...
        return nodes_time[['osmid', f'time_{poi_name}', f'{poi_name}_{count_pois[1]}min',
                          'x', 'y', 'geometry']]
    else:
...
def _prepare_network_edges(edges, prox_measure, walking_speed, projected_crs):
    ...
    if missing_length > 0:
        ...
        edges.loc[edges['length'].isna(), 'length'] = edges_projected.loc[edges['length'].isna()].length
...
def _calculate_poi_distances_batch(nearest, nodes, edges, poi_name, count_pois, progress_callback):
    ...
    for i in range(n_batches):
        ...
        batch_result = calculate_distance_nearest_poi(
            batch, nodes_analysis, edges, poi_name, 'osmid',
            weight='time_min', count_pois=count_pois
        )
...
def _process_geometry_groups(nearest, nodes, goi_id, max_walking_distance):
    ...
    log(f"Filtered POIs to {len(nearest_filtered)} within {max_walking_distance}m of network")
    return nearest_filtered


def _calculate_poi_distances_by_geometry(nearest, nodes, edges, poi_name, goi_id,
                                        count_pois, progress_callback):
    ...
    for i, goi in enumerate(unique_geometries):
        ...
        result = calculate_distance_nearest_poi(
            geometry_pois, nodes_analysis, edges, poi_name, 'osmid',
            weight='time_min', count_pois=count_pois
        )
